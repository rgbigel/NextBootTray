...
- Install-NextBootTray.ps1: installs runtime files to D:\OneDrive\cmd and writes HKCU Run startup entry.
...
- Uses Start-Process with -Verb RunAs to run elevated pwsh.
...
- Classify-BcdSection:
  ...
  - Applies pattern-based classification into Windows, Linux, rEFInd, Tool, Boot.
...
- NextBoot-BootNow.ps1:
  - Executes bcdedit /bootsequence <GUID> then shutdown /r /t 0.

### 2.5 Installation and Startup Registration
- Install-NextBootTray.ps1 copies executable scripts, icon, and docs to D:\OneDrive\cmd.
- Installer writes HKCU Run value NextBootTray = "D:\OneDrive\cmd\NextBootTray.cmd".
- Startup registration is per-user and runs at logon.
...
3. On normal startup, tray script performs self-clean of older tray instances.
4. Tray icon is shown and right-click exit menu is attached.
5. Left-click triggers Build-LeftMenu, which refreshes BCD-derived state and rebuilds items.
6. User selection triggers either set-default action or boot-now action.
7. Message loop runs until exit; resources are disposed in finally block.
...
- In-memory script scope state:
  ...
  - BootCacheReady
  - IsOpeningLeftMenu
- No persistent runtime state is written by tray script except optional debug log file.
